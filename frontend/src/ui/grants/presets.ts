...
//
//   一条预设 = 若干**现有** cap 的授予组合 + 一个统一 TTL。本模块是纯数据 +
//   纯函数（零 import、零 DOM，可在 node 里直接加载做断言），因此：
 ...
 * 四个一键组合（顺序即面板展示顺序）。
 *
 * 语义注意（安全面，见报告）：后端的 `net_hosts` 是**并集放宽**（并入站点放行清单），
...
export const PRESETS: readonly GrantPreset[] = [
  {
    id: 'read-workspace',
    label: '只读工作区',
    hint: '选一个目录（建议选本会话的工作区）读取其中文件，不能修改。',
    ttlSec: 1800,
  ...
  },
  {
    id: 'write-output',
    label: '写输出目录',
    hint: '选一个目录（建议选工作区的输出目录）在其中创建与修改文件。',
    ttlSec: 1800,
  ...
  },
  {
    id: 'net-15min',
    label: '临时联网',
    hint: '访问互联网与内网（含本机服务）；15 分钟后自动失效。',
    ttlSec: 900,
  ...
  },
  {
    id: 'localhost',
    label: '本机服务',
    hint: '访问网络，并把 localhost 与 127.0.0.1 放进站点放行清单。',
    ttlSec: 1800,
    steps: [
...
/** 某项能力在本预设下的有效期：统一 TTL 与服务端上限取小（上限缺失/非法 = 不收敛）。 */
